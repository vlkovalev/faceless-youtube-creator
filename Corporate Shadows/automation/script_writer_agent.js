...
- Target 1100–1500 total words for an 8–11 minute video.
...
function saintsPrompt(topic) {
  return `Write a complete The Saints script for this topic:
...
Required scene structure (write ALL scenes, minimum 6):
1. The Opening — one vivid human detail: a moment, a practice, a place. Make the saint feel real.
2. Origins — early life, call to religious life, formative experiences
3. The Ministry — what the saint did, how they served, key works or writings
4. The Trial — the hardship, temptation, persecution, or spiritual struggle they faced
5. The Miracle or Turning Point — a key spiritual event (flag as tradition vs documented history)
6. The Legacy — how they influenced the Church, their canonization, relics, veneration
7. The Takeaway — what a modern Orthodox Christian (or any viewer) can learn

Output this exact JSON structure (no markdown wrappers):
{
  "video": { "title": "...", "niche": "Orthodox Christian History", "duration": "..." },
  "scenes": [
    {
      "scene_number": 1,
      "title": "...",
...
function loadTopic(channel, topicId) {
  if (channel === 'cs') {
    const config = JSON.parse(fs.readFileSync(path.join(ROOT, 'channel_config.json'), 'utf8'));
...
function writeScript(channel, topicId, scriptJson) {
  ...
  if (channel === 'saints') {
    const outPath = path.join(SCRIPTS_DIR, `saints_video_${topicId}_data.js`);
...
async function main() {
  ...
  if (!hasKey && !DRY_RUN) {
    console.error('❌  No LLM API keys set. Please set GEMINI_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY in Corporate Shadows/automation/credentials/.env');
    process.exit(1);
  }
