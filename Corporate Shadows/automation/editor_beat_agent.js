...
function renderImageClip(imagePath, duration, outPath, zoomSeed) {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg()
      ...
      .complexFilter([
        `[0:v]scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,${zoom},eq=contrast=1.12:saturation=0.88:brightness=0.03,vignette=angle=0.3[v]`
      ])
...
async function renderScene(scenePlan, scriptScene, sceneIndex) {
  ...
  if (!fs.existsSync(audioPath)) {
    audioPath = path.join(ASSETS_DIR, `scene_${String(sceneIndex).padStart(2, '0')}_audio.wav`);
  }
  if (!fs.existsSync(audioPath)) {
    audioPath = path.join(ASSETS_DIR, `scene_${sceneIndex}_audio.mp3`);
  }
  if (!fs.existsSync(audioPath)) {
    audioPath = path.join(ASSETS_DIR, `scene_${String(sceneIndex).padStart(2, '0')}_audio.mp3`);
  }
  if (!fs.existsSync(audioPath)) throw new Error(`Missing audio: scene_${sceneIndex}_audio.wav/.mp3 or padded zero version`);
