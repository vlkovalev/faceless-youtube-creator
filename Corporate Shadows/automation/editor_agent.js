...
for (let i = 1; i <= 12; i++) {
    const audioCandidates = [
        path.join(ASSETS_DIR, `scene_${i}_audio.wav`),
        path.join(ASSETS_DIR, `scene_${String(i).padStart(2, '0')}_audio.wav`),
        path.join(ASSETS_DIR, `scene_${i}_audio.mp3`),
        path.join(ASSETS_DIR, `scene_${String(i).padStart(2, '0')}_audio.mp3`)
    ];
...
async function createSceneVideo(scene) {
    return new Promise((resolve, reject) => {
        ...
        if (scene.type === 'video') {
            ...
            filterStr = `[0:v]scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,zoompan=z='min(zoom+0.00035,1.12)':d=${totalFrames}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1920x1080:fps=30`;
        }
