...
function checkReadiness(id) {
  ...
  const report = [
    ...
    { check: 'Scene audio files', ok: countFiles(assetsDir, isSceneAudioAsset) >= 12, detail: `${countFiles(assetsDir, isSceneAudioAsset)}/12 audio files (.wav/.mp3)` },
    { check: 'Scene visual files', ok: countFiles(assetsDir, name => /^scene_\d+_(image\.png|video\.mp4)$/.test(name) || /^beat_\d+[a-z]_real\.(jpg|jpeg|png)$/.test(name)) >= 12, detail: `${countFiles(assetsDir, name => /^scene_\d+_(image\.png|video\.mp4)$/.test(name) || /^beat_\d+[a-z]_real\.(jpg|jpeg|png)$/.test(name))}/12 visual files` },
...
function runStage(stageName) {
  ...
  if (stageName === 'edit') {
    ...
    if (beatVideos.includes(String(videoId))) {
      runNodeScript('editor_beat_agent.js', videoId);
    } else {
