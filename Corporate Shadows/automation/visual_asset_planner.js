...
// ---------------------------------------------------------------------------
// Corporate Shadows visual formula — loaded from the canonical template
// (also enforced by agents/visual_formula_agent.md). Acts are stored here as
// fractions of total runtime so they scale to each script's actual length.
// Shared with broll_shortlist_generator.js via formula_acts.js so both read
// the same canonical data and cannot drift apart.
// ---------------------------------------------------------------------------

var formulaActsModule = require('./formula_acts');
var FORMULA_ACTS = formulaActsModule.loadFormulaActs();

function actForFraction(frac) {
    return formulaActsModule.actForFraction(FORMULA_ACTS, frac);
}
...
function buildQuery(beatText, srcType, act) {
    ...
    switch (srcType) {
        ...
        case SRC.GENERATED: return '[GENERATE] ' + core + ' — ' + (act ? act.aiPromptStyle : 'dark corporate documentary style');
        case SRC.STOCK:     return core + ' stock footage';
        default:            return core;
    }
}
...
function buildUrl(query, srcType) {
    var term = encodeURIComponent(
        query.replace(/\s*site:[^\s]+/g, '').replace(/\[GENERATE\]\s*/g, '').replace(/\s*stock footage\s*$/i, '').trim()
    );
    switch (srcType) {
        ...
        case SRC.STOCK:     return 'https://www.pexels.com/search/videos/' + term + '/';
        default:            return null;
    }
}

// Stock b-roll has no single canonical free API without a key, so beats of
// this type also get Pixabay/Mixkit alternates alongside the primary Pexels URL.
function buildStockAlternates(query) {
    var term = encodeURIComponent(
        query.replace(/\s*stock footage\s*$/i, '').trim()
    );
    return {
...
function buildPlan(videoId, data) {
    // Pass 1: estimate durations to learn total runtime before classifying acts.
    var durations = data.scenes.map(function(scene) { return estimateDuration(scene.voiceover); });
    var totalDuration = durations.reduce(function(a, b) { return a + b; }, 0);

    var runTime = 0;
    var planScenes = [];

    data.scenes.forEach(function(scene, sceneIndex) {
        var dur = durations[sceneIndex];
        var act = actForFraction(totalDuration > 0 ? runTime / totalDuration : 0);
        ...
        var beats = beatTexts.map(function(text, i) {
            var beatAct = actForFraction(totalDuration > 0 ? timings[i].start / totalDuration : 0);
            var rec = recommendSource(text, scene.title, scene.visual_prompt);
            var query = buildQuery(text, rec.type, beatAct);
            ...
            return {
                ...
                source_url: url,
                source_alternates: rec.type === SRC.STOCK ? buildStockAlternates(cleanQuery) : null,
                asset_file: null,
                status: 'pending',
        ...
        planScenes.push({
            ...
            act: {
                ...
                name: act.name,
                b_roll: act.bRoll,
                sound_design: act.soundDesign,
                camera_style: act.cameraStyle,
            },
            beats: beats,
        });
    ...
    return {
        ...
        visual_formula: FORMULA_ACTS.map(function(a) {
            return {
                key: a.key,
                name: a.name,
                start_s: Math.round(a.startFrac * runTime),
                end_s: Math.round(a.endFrac * runTime),
            };
        }),
        scenes: planScenes,
    };
...
function printSummary(videoId, plan) {
    ...
    console.log('\nVisual formula acts:');
    plan.visual_formula.forEach(function(a) {
        console.log('  ' + pad(a.name, 18) + a.start_s + 's–' + a.end_s + 's');
    });
