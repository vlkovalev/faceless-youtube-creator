...
function buildQueueEntry(id, existingEntry = {}) {
  ...
  return {
    ...
    status: existingEntry.status || 'scheduled',
    publish_days: existingEntry.publish_days || cadence.days || ['Tuesday', 'Friday'],
    publish_time: existingEntry.publish_time || cadence.time || '10:00',
    timezone: existingEntry.timezone || cadence.timezone || 'America/Denver',
    thumbnail_test: {
...
function upsertQueueEntry(id) {
  ...
  const existingEntry = queue.find(item => item.filename === `FINAL_VIDEO_${id}.mp4`) || {};
  const entry = buildQueueEntry(id, existingEntry);
