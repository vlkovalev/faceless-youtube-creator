...
const REPORT_JSON = path.join(METADATA_DIR, 'comment_moderation_report.json');
const REPORT_MD = path.join(METADATA_DIR, 'comment_moderation_report.md');
...
const CHANNEL_PROFILE = (args.channel || 'cs').toLowerCase();
const IS_LIVE = args.live === true || args.live === 'true';
const DRY_RUN = !IS_LIVE;

const TOKENS_FILE = path.join(
  CHANNEL_PROFILE === 'saints' ? SAINTS_CREDENTIALS_DIR : path.join(__dirname, 'credentials'),
...
const STATUS_FILE = path.join(
  CHANNEL_PROFILE === 'saints' ? SAINTS_METADATA_DIR : METADATA_DIR,
...
async function run() {
  ...
  const channelName = statusData.channel ? statusData.channel.title : (CHANNEL_PROFILE === 'saints' ? 'The Saints' : 'Corporate Shadows');
  ...
  for (const thread of threads) {
    ...
    let analysis = {
      ...
      flagged: false,
      flagged_reason: null,
      reply_draft: 'Thank you for watching and supporting the channel!'
    };
    ...
    if (hasKey) {
      try {
        ...
        if (commentText.toLowerCase().includes('wrong') || commentText.toLowerCase().includes('fake') || commentText.toLowerCase().includes('robotic')) {
          analysis = {
            category: 'complaint',
            flagged: true,
