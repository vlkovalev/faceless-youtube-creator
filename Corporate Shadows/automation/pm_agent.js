...
function createDefaultDependencyRules() {
  const rules = [
    ...
    { id: 'dep-custom-thumbnails', channel: 'Corporate Shadows', severity: 'P2', rule: 'Custom thumbnails require YouTube account permission/verification.', check: 'YouTube upload warning', status: 'open' }
  ];
...
function buildRaid(classified, production, blockers, sensitiveFiles, dependencyRules) {
  ...
  assumptions.push('Corporate Shadows is the active/live channel.');
...
function buildReport() {
  ...
  const gateCounts = gateStatus && gateStatus.corporate_shadows
    ? gateStatus.corporate_shadows
    : null;
  const hasGateCounts = gateCounts &&
    Number.isFinite(gateCounts.upgraded_public_videos) &&
    Number.isFinite(gateCounts.scheduled_upgraded_videos);
  const gateSummary = hasGateCounts
    ? ` Corporate Shadows upgraded videos: ${gateCounts.upgraded_public_videos} public, ${gateCounts.scheduled_upgraded_videos} scheduled.`
    : '';
